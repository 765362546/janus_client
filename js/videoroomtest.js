...
var iceUser = null;
var icePass = null;
function set_janus_server() {
	server = $("#server").val();
	localStorage.setItem("server", server);
	console.log("--------------------janus_server: " + server)
}
function set_iceserver() {
	iceServer = $("#iceServer").val();
	iceUser = $("#iceUser").val();
	icePass = $("#icePass").val();
	localStorage.setItem("iceserver", iceServer);
	localStorage.setItem("iceuser", iceUser);
	localStorage.setItem("icepass", icePass);
	console.log("-------------------ice_server: " + iceServer)
}

var myroom = null;	// Demo room
function set_myroom() {
	myroom = Number($("#myroom").val())
	if (!myroom) {
		alert('房间id必须为数字')
...
$(document).ready(function () {
	...
	var tmp_server=localStorage.getItem("server")
	var tmp_room=localStorage.getItem("myroom")
	var tmp_iceserver=localStorage.getItem("iceserver")
	var tmp_iceuser=localStorage.getItem("iceuser")
	var tmp_icepass=localStorage.getItem("icepass")
	if(tmp_server){
		$("#server").val(tmp_server)
	}
	if( ! isNaN(tmp_room)) {
		$("#myroom").val(tmp_room)
	}
	if(tmp_iceserver){
		$("#iceServer").val(tmp_iceserver)
	}
	if(tmp_iceuser){
		$("#iceUser").val(tmp_iceuser)
	}
	if(tmp_icepass){
		$("#icePass").val(tmp_icepass)
	}
	...
	Janus.init({
		...
		$('#start').one('click', function() {
			...
			janus = new Janus(
				{
					server: server,
					iceServers: [{urls: iceServer, username: iceUser, credential: icePass}],
					success: function() {
